<?php
/**
 * export_excel_spv.php
 * Letakkan file ini di: /spv/export_excel_spv.php
 *
 * SPV dapat mengexport data semua cabang (sama seperti owner).
 *
 * Parameter GET:
 *   filter_type : mingguan | bulanan | tahunan | custom
 *   tgl_dari    : Y-m-d   (wajib jika custom)
 *   tgl_sampai  : Y-m-d   (wajib jika custom)
 *   detail_uid  : int     → export 1 staf saja
...
// ── Scope export (sama persis seperti owner) ───────────────────────────────
...
function applyStyle($sheet, $range, array $opt): void
{
    ...
    if (!empty($opt['bg']))    $s->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('FF' . $opt['bg']);
    ...
    if (!empty($opt['border'])) {
        $bc = $opt['bc'] ?? 'E5E5EA';
        $s->getBorders()->applyFromArray(['allBorders' => [
            'style' => Border::BORDER_THIN, 'color' => ['argb' => 'FF' . $bc]
        ]]);
    }
}
...
{
    return match($s) {
        'approved' => 'Diterima', 'rejected' => 'Ditolak', 'pending' => 'Pending', default => '-',
    };
}
...
$spreadsheet->getProperties()
    ...
    ->setTitle('Laporan Absensi SPV – ' . $label_display)
    ->setSubject('Export SPV – ' . $scope_label)
    ->setDescription('Diekspor oleh SPV: ' . $spv_nama . ' pada ' . date('d M Y H:i'));

$firstSheet = true;

// ── Helper: buat sheet detail per cabang ──────────────────────────────────
function buildDetailSheet(
    Spreadsheet $spreadsheet, array $branchData, string $tgl_dari, string $tgl_sampai,
    string $label_display, bool &$firstSheet, $pdo, string $spv_nama,
    string $C_DARK, string $C_DARK2, string $C_PRIMARY, string $C_PRIMARY_L,
    string $C_SUCCESS, string $C_DANGER, string $C_WARNING,
    string $C_ALT_ROW, string $C_BORDER, string $C_WHITE, string $C_SPV_HDR
): void {
    $b         = $branchData['branch'];
    ...
    applyStyle($ws, 'A1:L1', ['bold' => true, 'size' => 13, 'fg' => $C_WHITE, 'bg' => $C_SPV_HDR,
        'ha' => Alignment::HORIZONTAL_CENTER, 'va' => Alignment::VERTICAL_CENTER]);
    $ws->getRowDimension(1)->setRowHeight(30);

    $ws->mergeCells('A2:L2');
    $ws->setCellValue('A2', 'Periode: ' . $label_display . '   |   Diekspor oleh SPV: ' . $spv_nama . '   |   ' . date('d M Y H:i'));
        ...
        if (!$abs_rows) {
            ...
            foreach ($abs_rows as $a) {
                ...
                $apColor = match($a['status_alasan']) {
                    'approved' => $C_SUCCESS, 'rejected' => $C_DANGER, 'pending' => $C_WARNING, default => '8E8E93',
                };
            ...
            $ws->setCellValue("K{$row}", 'Credit Score: ' . $u['credit_score']);
...
if (count($scope) > 1) {
    ...
    applyStyle($ws, 'A1:J1', ['bold' => true, 'size' => 14, 'fg' => $C_WHITE, 'bg' => $C_SPV_HDR,
        'ha' => Alignment::HORIZONTAL_CENTER, 'va' => Alignment::VERTICAL_CENTER]);
...
foreach ($scope as $item) {
    buildDetailSheet(
        $spreadsheet, $item, $tgl_dari, $tgl_sampai, $label_display, $firstSheet, $pdo, $spv_nama,
        $C_DARK, $C_DARK2, $C_PRIMARY, $C_PRIMARY_L,
        $C_SUCCESS, $C_DANGER, $C_WARNING,
        $C_ALT_ROW, $C_BORDER, $C_WHITE, $C_SPV_HDR
    );
}
...
// ── Output ─────────────────────────────────────────────────────────────────
$filename = 'Absensi_SPV_' . $scope_label . '_' . $label . '_' . date('Ymd_His') . '.xlsx';
