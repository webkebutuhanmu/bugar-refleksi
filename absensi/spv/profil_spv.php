...
<div class="card slide-up">
    <div class="card-title"><i class="fas fa-user-circle" style="color:var(--primary)"></i> Profil Akun SPV</div>
            ...
            <div style="display:flex; align-items:center; gap:12px;">
                ...
                <div style="flex:1;">
                    <input type="range" id="sliderZoom" min="0.5" max="3" step="0.05" value="1"
                        oninput="setZoom(parseFloat(this.value))" style="width:100%; accent-color:#5856D6;">
                </div>
                <button onclick="zoomFoto(0.1)" style="background:#E5E5EA; border:none; width:36px; height:36px; border-radius:50%; cursor:pointer; font-size:16px; font-weight:800;">+</button>
            </div>
...
function toggleEditProfil() {
    ...
    if (wrap.style.display === 'none') {
        ...
        btn.style.background = '#8E8E93'; btn.style.boxShadow = 'none';
    } else {
        wrap.style.display = 'none';
        btn.innerHTML = '<i class="fas fa-pencil-alt"></i> Edit Profil';
        btn.style.background = 'var(--primary)'; btn.style.boxShadow = '0 4px 15px rgba(88,86,214,0.25)';
    }
}
...
function konfirEditProfil() {
    ...
    Swal.fire({ title: 'Simpan Perubahan?', text: 'Data profil akan diperbarui.', icon: 'question',
...
let isDragging=false,dragStartX=0,dragStartY=0,dragOX=0,dragOY=0,lastPinchDist=0;

async function bukaModalCrop(input) {
    if (!input.files || !input.files[0]) return;
    let file = input.files[0];

    // Deteksi HEIC dari iOS
    ...
    reader.onload=function(e){
        ...
        cropImg.onload=function(){
            ...
            const scaleW=canvas.width/cropImg.width; const scaleH=canvas.height/cropImg.height;
...
function zoomFoto(delta){
    ...
    slider.value=newVal; setZoom(newVal);
}

function startDrag(e){isDragging=true;dragStartX=e.clientX;dragStartY=e.clientY;dragOX=cropX;dragOY=cropY;document.getElementById('cropArea').style.cursor='grabbing';e.preventDefault();}
document.addEventListener('mousemove',e=>{if(!isDragging)return;cropX=dragOX+(e.clientX-dragStartX);cropY=dragOY+(e.clientY-dragStartY);renderCrop();});
document.addEventListener('mouseup',()=>{isDragging=false;const a=document.getElementById('cropArea');if(a)a.style.cursor='grab';});
function startDragTouch(e){if(e.touches.length===1){isDragging=true;dragStartX=e.touches[0].clientX;dragStartY=e.touches[0].clientY;dragOX=cropX;dragOY=cropY;}}
document.addEventListener('touchmove',e=>{
    if(e.touches.length===2){const d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);if(lastPinchDist){zoomFoto((d-lastPinchDist)*0.005);}lastPinchDist=d;isDragging=false;}
    else if(isDragging&&e.touches.length===1){cropX=dragOX+(e.touches[0].clientX-dragStartX);cropY=dragOY+(e.touches[0].clientY-dragStartY);renderCrop();}
},{passive:true});
document.addEventListener('touchend',()=>{isDragging=false;lastPinchDist=0;});
document.getElementById('cropArea')&&document.getElementById('cropArea').addEventListener('wheel',e=>{e.preventDefault();zoomFoto(e.deltaY<0?0.07:-0.07);},{passive:false});
...
function simpanFoto(){
    ...
    out.width=out.height=size; const ctx=out.getContext('2d');
    
    const areaRect=area.getBoundingClientRect();
    const guideRect=guide.getBoundingClientRect();
    const gx=guideRect.left-areaRect.left; const gy=guideRect.top-areaRect.top;
    ...
    Swal.fire({title:'Gunakan foto ini?',icon:'question',showCancelButton:true,confirmButtonColor:'#5856D6',cancelButtonText:'Batal',confirmButtonText:'Ya, Simpan'
