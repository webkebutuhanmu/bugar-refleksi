...
<div class="card slide-up">
    <div class="card-title">
        <i class="fas fa-fingerprint" style="color:var(--primary)"></i> Presensi Sistem
    </div>
    ...
    <?php if (!$absen): ?>
        ...
        <form id="formAbsen" action="../proses.php?action=absen_masuk" method="POST">
            ...
            <div style="text-align:center; padding:20px; background:#F2F2F7; border-radius:15px; margin-bottom:20px;">
                ...
                <div style="font-size:13px; color:#1C1C1E;">
                    Batas Toleransi: <b style="color:var(--danger)"><?= date('H:i', strtotime($batas_toleransi)) ?></b>
                </div>
            </div>

            <?php if ($is_late): ?>
                <div style="background:#FFF5E5; border-left:4px solid var(--warning); padding:15px; border-radius:10px; margin-bottom:15px;">
                    <div style="font-weight:800; color:#856404; font-size:14px;">
                        <i class="fas fa-exclamation-circle"></i> ANDA TERLAMBAT
                    </div>
                    <p style="margin:5px 0 0; font-size:12px; color:#856404; line-height:1.4;">
                        Credit score Anda berkurang 5 poin. Isi alasan agar SPV dapat mengembalikan skor Anda.
                    </p>
                </div>
                <textarea name="alasan" id="alasanTeks" placeholder="Tulis alasan keterlambatan..." style="width:100%; border-radius:12px; padding:15px; border:1px solid rgba(0,0,0,0.1); margin-bottom:20px; font-size:14px; background:#F9F9F9; outline:none; box-sizing:border-box;" rows="3" required></textarea>
            <?php endif; ?>

            <button type="button" onclick="konfirAbsen('masuk', <?= $is_late ? 'true' : 'false' ?>)" style="background:<?= $is_late ? 'var(--warning)' : 'var(--success)' ?>; color:white; border:none; width:100%; padding:15px; border-radius:12px; font-size:16px; font-weight:bold; cursor:pointer; margin-bottom:15px;">
                <i class="fas fa-sign-in-alt"></i> <?= $is_late ? 'Absen & Kirim Alasan' : 'Absen Masuk' ?>
            </button>
        </form>
    ...
    <?php elseif ($absen && in_array($absen['status_kehadiran'], ['Sakit', 'Izin'])): ?>
        <div style="text-align:center; padding:30px 0;">
            ...
            <div style="font-size:13px; color:#8E8E93; margin-bottom:20px;">
                Anda telah mengajukan <?= $absen['status_kehadiran'] ?> untuk hari ini.
            </div>
    ...
    <?php elseif ($absen && $absen['waktu_keluar'] == NULL): ?>
        ...
        <div style="text-align:center; padding:30px 0;">
            ...
            <div style="font-size:28px; font-weight:800; color:var(--text); margin-bottom:10px;"><?= $absen['waktu_masuk'] ?></div>
            <span class="status-pill <?= $absen['status_kehadiran'] == 'Tepat Waktu' ? 'pill-tepat' : 'pill-telat' ?>" style="margin-bottom:25px; display:inline-block;">
                <?= $absen['status_kehadiran'] ?>
            </span>
            <button onclick="konfirAbsen('pulang', false)" style="background:var(--danger); color:white; border:none; width:100%; padding:15px; border-radius:12px; font-size:16px; font-weight:bold; cursor:pointer;">
    ...
    <?php else: ?>
        ...
        <div style="text-align:center; padding:30px 0;">
            <div style="width:70px; height:70px; background:#E2F9E9; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 15px;">
            ...
            <div style="font-size:13px; color:#8E8E93;">
                <span style="color:var(--success);"><?= $absen['waktu_masuk'] ?></span>
                <i class="fas fa-arrow-right" style="font-size:10px; margin:0 5px;"></i>
                <span style="color:var(--danger);"><?= $absen['waktu_keluar'] ?></span>
            </div>
        </div>
...
function konfirAbsen(jenis, isLate) {
    if (jenis === 'masuk' && isLate) {
        ...
        if (!el || el.value.trim().length < 5) return Swal.fire('Gagal', 'Alasan minimal 5 karakter!', 'error');
    }
    Swal.fire({
        title: 'Konfirmasi',
        text: jenis === 'masuk' ? (isLate ? 'Absen akan dicatat sebagai TERLAMBAT.' : 'Lanjutkan absen masuk?') : 'Lanjutkan absen pulang?',
        icon: 'question',
...
function ajukan(jenis, shift) {
    Swal.fire({
        ...
        html: `<p style="font-size:13px; color:var(--danger); margin-top:5px;">Perhatian: Credit score akan terpotong 5 poin. Skor dikembalikan jika disetujui SPV.</p>`,
        showCancelButton: true,
        confirmButtonText: 'Kirim Pengajuan',
        cancelButtonText: 'Batal',
        confirmButtonColor: '#5856D6',
        preConfirm: (alasan) => {
            if (!alasan || alasan.trim().length < 5) Swal.showValidationMessage('Alasan detail minimal 5 karakter!');
            return alasan;
        }
    }).then((res) => {
        if (res.isConfirmed) {
