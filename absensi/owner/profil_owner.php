...
// Statistik global untuk owner
$total_staf    = $pdo->query("SELECT COUNT(*) FROM users WHERE role != 'owner'")->fetchColumn();
$total_cabang  = $pdo->query("SELECT COUNT(*) FROM branches")->fetchColumn();
$total_terlambat = $pdo->query("SELECT COUNT(*) FROM attendance WHERE status_kehadiran = 'Terlambat'")->fetchColumn();
?>

<div class="card slide-up">
    <div class="card-title"><i class="fas fa-user-circle" style="color:var(--primary)"></i> Profil Akun Owner</div>
    ...
    <div style="display:flex; align-items:center; gap:20px; padding:20px; background:linear-gradient(135deg, #1C1C1E, #3A3A3C); border-radius:16px; margin-bottom:20px; color:white;">
        ...
        <div style="position:relative; width:72px; height:72px; flex-shrink:0;">
            ...
            <img id="avatarImg" src="../<?= htmlspecialchars($me['foto_profil']) ?>?v=<?= time() ?>"
                 ...
                 style="width:72px; height:72px; border-radius:50%; object-fit:cover; border:3px solid rgba(255,215,0,0.5); cursor:pointer;">
            <?php else: ?>
            <!-- Jika belum ada foto: klik ikon akan membuka pilih foto -->
            <div id="avatarImg" onclick="document.getElementById('inputFoto').click()" style="width:72px; height:72px; background:rgba(255,255,255,0.15); border-radius:50%; display:flex; align-items:center; justify-content:center; border:3px solid rgba(255,215,0,0.5); cursor:pointer;">
                <i class="fas fa-crown" style="font-size:28px; color:#FFD700;"></i>
            </div>
            <?php endif; ?>
        ...
        <div>
            ...
            <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:6px;">
                <span style="background:rgba(255,215,0,0.2); color:#FFD700; padding:4px 12px; border-radius:20px; font-size:11px; font-weight:800; text-transform:uppercase; border:1px solid rgba(255,215,0,0.3);">
...
<div class="card slide-up delay-1">
    <div class="card-title"><i class="fas fa-chart-pie" style="color:var(--primary)"></i> Statistik Sistem</div>
    <div style="display:flex; gap:12px; flex-wrap:wrap;">
        <div style="flex:1; min-width:100px; background:linear-gradient(135deg,#5856D6,#7E7CE6); border-radius:14px; padding:18px 15px; text-align:center; color:white;">
            <div style="font-size:28px; font-weight:800;"><?= $total_staf ?></div>
        ...
        <div style="flex:1; min-width:100px; background:linear-gradient(135deg,#34C759,#30D158); border-radius:14px; padding:18px 15px; text-align:center; color:white;">
            <div style="font-size:28px; font-weight:800;"><?= $total_cabang ?></div>
            <div style="font-size:11px; font-weight:700; opacity:0.9; margin-top:3px;">TOTAL CABANG</div>
        </div>
        <div style="flex:1; min-width:100px; background:linear-gradient(135deg,#FF3B30,#FF6B60); border-radius:14px; padding:18px 15px; text-align:center; color:white;">
            <div style="font-size:28px; font-weight:800;"><?= $total_terlambat ?></div>
            <div style="font-size:11px; font-weight:700; opacity:0.9; margin-top:3px;">TOTAL TERLAMBAT</div>
        </div>
    </div>
</div>
...
function simpanFoto() {
    ...
    if (avatarEl.tagName === 'IMG') {
        ...
        img.style.cssText = 'width:72px;height:72px;border-radius:50%;object-fit:cover;border:3px solid rgba(255,215,0,0.5); cursor:pointer;';
