...
if ($shift_aktif === 2 && $t_skrg < $t_s1_mul) { $is_late = true; } else { $is_late = $t_skrg > strtotime($batas_toleransi); }
?>

<div class="card slide-up">
    <div class="card-title">
        <i class="fas fa-fingerprint" style="color:var(--primary)"></i> Presensi Admin
    </div>
    ...
    <?php if (!$absen): ?>
        <form id="formAbsen" action="../proses.php?action=absen_masuk" method="POST">
            ...
            <button type="button" onclick="konfirAbsen('masuk')" style="background:var(--success); color:white; border:none; width:100%; padding:15px; border-radius:12px; font-size:16px; font-weight:bold; cursor:pointer; margin-bottom:15px;"><i class="fas fa-sign-in-alt"></i> Absen Masuk</button>
        </form>

    <?php elseif ($absen && in_array($absen['status_kehadiran'], ['Sakit', 'Izin'])): ?>
        <div style="text-align:center; padding:30px 0;">
            <div style="width:70px; height:70px; background:#FFF5E5; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 15px;"><i class="fas <?= $absen['status_kehadiran'] == 'Sakit' ? 'fa-notes-medical' : 'fa-envelope-open-text' ?>" style="font-size:30px; color:var(--warning);"></i></div>
    ...
    <?php elseif ($absen && $absen['waktu_keluar'] == NULL): ?>
        <div style="text-align:center; padding:30px 0;">
            ...
            <button onclick="konfirAbsen('pulang')" style="background:var(--danger); color:white; border:none; width:100%; padding:15px; border-radius:12px; font-size:16px; font-weight:bold; cursor:pointer;"><i class="fas fa-sign-out-alt"></i> Absen Pulang</button>
        </div>
...
function konfirAbsen(jenis) {
    Swal.fire({
        title: 'Konfirmasi', text: jenis === 'masuk' ? 'Lanjutkan absen masuk?' : 'Lanjutkan absen pulang?',
        icon: 'question', showCancelButton: true, confirmButtonColor: jenis === 'masuk' ? '#34C759' : '#FF3B30',
        cancelButtonColor: '#E5E5EA', cancelButtonText: '<span style="color:#1C1C1E;font-weight:bold;">Batal</span>', confirmButtonText: 'Ya, Lanjutkan'
    }).then((r) => {
        if (r.isConfirmed) {
